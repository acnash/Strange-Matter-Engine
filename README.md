...
The current candidate is **Endpoint-Aligned Cross-Validated CYP-Specialist Graph Cellular Automata (EA-CV-CYP-GCA v1)**. Four independent nonlinear systems specialise the cellular dynamics for CYP1A2, CYP2C9, CYP2D6, and CYP3A4. Each backpropagation batch, differentiable ridge solve, early-stopping decision, and validation score uses observations from its active CYP endpoint. All ten transition rules were screened for every endpoint, followed by five-fold, two-seed confirmation and sparse rule selection. CYP1A2 selected FitzHugh-Nagumo, Gray-Scott, and conservative graph flux; CYP2C9 selected Gray-Scott and damped symplectic; CYP2D6 selected delayed memory and FitzHugh-Nagumo; CYP3A4 selected damped symplectic, FitzHugh-Nagumo, and delayed memory.

Every specialist retains backpropagation through the complete Graph-CA trajectory and a genuine differentiable ridge readout. Rule predictions are averaged across confirmed scaffold folds and seeds, then combined by the saved endpoint-specific ridge state. The sealed validation holdout remained outside screening, hyperparameter promotion, rule selection, and ridge fitting.

On the sealed validation set, EA-CV-CYP-GCA achieved point MA-ST-RAE **0.7545**, bootstrap mean **0.7551** with a 95% interval of **0.7177 to 0.7970**, and RMSE **0.8523 pIC50**. Its bootstrap macro MAE was 0.6218, R-squared 0.2864, Spearman rho 0.5327, and Kendall tau 0.3816. This improved the preceding CV-CYP-GCA point MA-ST-RAE of 0.7690 and RMSE of 0.8625.

The current 1 September leaderboard snapshot places the CV-CYP-GCA submission 99th of 111 with MA-ST-RAE 1.0171, macro MAE 1.0848, macro R-squared -0.0840, macro Spearman rho 0.5180, and macro Kendall tau 0.3637. These organiser results provide the external benchmark for EA-CV-CYP-GCA.
