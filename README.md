...
The current candidate is the **Cross-Fitted Target-Specific Dual-Scale Graph Cellular Automata Ensemble (CFT-DS-GCAE v1)**. It retains the original and multiscale graph-CA members from DS-GCAE v1, then replaces fixed final averaging with a separately regularised ridge stack for CYP1A2, CYP2C9, CYP2D6, and CYP3A4. Each stack receives ten cellular-automata predictions, one original and one multiscale signal from each of gated residual, delayed memory, inertial reaction diffusion, Kuramoto-Sakaguchi, and FitzHugh-Nagumo.

Nested scaffold-grouped cross-fitting selected the ridge strength and tested an optional affine calibration without exposing any prediction to parameters fitted using its fold. Calibration was retained only when it improved held-out MA-ST-RAE; the final search selected identity calibration for all four endpoints. The sealed validation holdout remained outside this process.

On the final validation set, CFT-DS-GCAE achieved point MA-ST-RAE **0.7739**, bootstrap mean **0.7749** with a 95% interval of **0.7356 to 0.8193**, and RMSE **0.8586 pIC50**. DS-GCAE achieved 0.7842 and 0.8678 respectively. The improvement is modest and the uncertainty intervals overlap.

The first DS-GCAE blind submission ranked 80th of 89, with leaderboard MA-ST-RAE 1.0132, MA 1.0893, macro R-squared -0.0827, macro Spearman rho 0.4751, and macro Kendall tau 0.3323. This generalisation gap motivated the cross-fitted target-specific stack. Frozen inference produced 3,000 finite CFT-DS-GCAE predictions for the 750 blinded molecules without loading test labels. The new challenge-ready file is [`results/production_cross_fitted_target_calibrated_gcae_v1/cft_ds_gcae_submission.csv`](results/production_cross_fitted_target_calibrated_gcae_v1/cft_ds_gcae_submission.csv).
...
Challenge-aligned v5 studies keep the original scaffold holdout sealed during hyperparameter search. Broad and surrogate-guided trials use scaffold folds drawn only from the fitting pool, successive halving promotes stable configurations, and leading configurations are confirmed across five folds and three seeds. Point MA-ST-RAE provides efficient epoch-level screening; finalists receive the official 1,000-resample bootstrap together with MAE, R2, Spearman rho, Kendall tau, and 95% intervals. All ten rules search the same chemistry profiles, including periodic, valence, electronic, ring, local-neighbour, and comprehensive atom descriptors.
