...
The sealed-validation leader is **USR-CIA-EA-CV-CYP-GCA v1**, an update-scale refinement of the credible-interval-aligned endpoint-specialist Graph-CA. It achieved point MA-ST-RAE **0.748039**, bootstrap mean **0.748534** with a 95% interval of **0.709822 to 0.789700**, and RMSE **0.845606 pIC50** on the fixed sealed scaffold holdout. CIA-EA-CV-CYP-GCA remains the external reference model for this refinement lineage, with sealed MA-ST-RAE 0.748490 and RMSE 0.847738 pIC50.

The official blind evaluation did not confirm the incremental sealed improvement. USR-CIA-EA-CV-CYP-GCA recorded rank **123 of 137**, MA-ST-RAE **1.0092**, macro MAE **1.0789**, macro R-squared **-0.0744**, macro Spearman rho **0.5345**, and macro Kendall tau **0.3762**. CIA-EA-CV-CYP-GCA recorded MA-ST-RAE 1.0075 and the strongest rank correlations in the current lineage, while EA-CV-CYP-GCA retains the lowest official primary metric among the recorded submissions at 1.0071.

These statuses remain distinct because the sealed holdout governed label-independent model selection and the official blind results provide external evaluation. Every retained system uses chemically encoded atoms as cells, typed bonded interactions, explicit recurrent generations, complete trajectory features, backpropagation, and a genuine differentiable ridge readout. The [permanent method report](METHOD_REPORT.md) provides the configuration, validation protocol, submission schema, and complete comparison.
