#!/usr/bin/env python3
"""Thorough, resumable coupled-map search over atom chemistry and CA controls."""
...
SEARCH_SEED = 240826
SEARCH_VERSION = "coupled_map_atom_features_ma_st_rae_v2"
STUDY_NAME = f"production_coupled_map_{SEARCH_VERSION}"
FEATURE_PROFILES = (
...
BASELINE_WINNER = {
    "generations": 16, "hidden_channels": 16, "ca_lr": 1e-3,
    "ridge": 1e-1, "ca_l2": 1e-5, "gradient_clip": 1.0,
    "batch_molecules": 64, "update_scale": 0.4, "init_scale": 0.5,
    "initial_noise": 0.0, "support_fraction": 0.85,
    "bond_temperature": 2.0, "dyn_a": 0.2, "dyn_b": 0.5,
    "dyn_c": 0.8, "dyn_d": 0.3, "atom_feature_profile": "baseline",
}


def candidates(count: int = 72) -> list[dict]:
    """Create the same balanced chemistry-first design used for Gray-Scott."""
...
def main() -> None:
    ...
    cache = ROOT / "tmp" / "coupled_map_atom_feature_graphs.pkl"
    ...
    for index, config in enumerate(design, 1):
        metrics = run_fit("coupled_map", STUDY_NAME, f"stage1_{index:03d}",
    ...
    for index, (config, _) in enumerate(promoted, 1):
        metrics = run_fit("coupled_map", STUDY_NAME, f"stage2_{index:02d}",
    ...
    for config_index, (config, _) in enumerate(finalists, 1):
        metrics_list = [run_fit(
            "coupled_map", STUDY_NAME, f"confirm_{config_index:02d}_seed_{seed}",
    ...
    if not viable:
        raise RuntimeError("Every coupled-map confirmation candidate failed")
    winner, winner_metrics, winner_mean, winner_sd, winner_score = min(
        viable, key=lambda item: item[4]
    )
    final_metrics = run_fit("coupled_map", STUDY_NAME, "final_model", winner,
    ...
    summary = {
        "study": STUDY_NAME, "rule": "coupled_map",
        "search_seed": SEARCH_SEED, "search_version": SEARCH_VERSION,
        ...
        "baseline_final_validation_rmse": 0.8788325190544128,
