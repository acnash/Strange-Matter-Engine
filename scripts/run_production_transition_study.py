...
def run_fit(rule, study_name, label, config, seed, epochs, patience,
    ...
    env.update({
        ...
        "SME_CHEMICAL_FEATURE_GATING": "1" if config.get("chemical_feature_gating", False) else "0",
        ...
        "SME_PERTURBATION_CONSISTENCY_WEIGHT": str(config.get("perturbation_consistency_weight", 0.0)),
        "SME_PERTURBATION_CONSISTENCY_EPSILON": str(config.get("perturbation_consistency_epsilon", 0.001)),
        "SME_INITIAL_STATE_ANCHOR": "1" if config.get("initial_state_anchor", False) else "0",
        "SME_DYNAMIC_OBSERVABLES": "1" if config.get("dynamic_observables", False) else "0",
        "SME_MULTISCALE_TRANSITION_ENERGY": "1" if config.get("multiscale_transition_energy", False) else "0",
        "SME_CHANNEL_ADAPTIVE_TIMESCALE": "1" if config.get("channel_adaptive_timescale", False) else "0",
        "SME_MULTILAG_RECURRENCE_SIGNATURE": "1" if config.get("multilag_recurrence_signature", False) else "0",
        "SME_TEMPORAL_EXTREMA_SIGNATURE": "1" if config.get("temporal_extrema_signature", False) else "0",
        "SME_DIRECTIONAL_FLUX_SIGNATURE": "1" if config.get("directional_flux_signature", False) else "0",
        "SME_ACTIVITY_BALANCE_STRENGTH": str(config.get("activity_balance_strength", 0.0)),
        "SME_BOND_MESSAGE_DROPOUT": str(config.get("bond_message_dropout", 0.0)),
        "SME_DEGREE_NORMALIZATION_POWER": str(config.get("degree_normalization_power", 1.0)),
