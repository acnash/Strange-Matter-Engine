...
def campaign_architecture() -> str:
    ...
    if CHANNEL_TIMESCALE_REFINEMENT:
        return "channel_adaptive_timescale_credible_interval_endpoint_graph_ca"
    if TRANSITION_ENERGY_REFINEMENT:
    ...
    if HIDDEN_WIDTH_REFINEMENT:
        return "hidden_width_refined_credible_interval_endpoint_graph_ca"
    if BOND_TEMPERATURE_REFINEMENT:
...
def campaign_submission_name() -> str:
    ...
    if CHANNEL_TIMESCALE_REFINEMENT:
        return "channel_adaptive_timescale_credible_interval_ea_cv_cyp_gca_submission.csv"
    if TRANSITION_ENERGY_REFINEMENT:
    ...
    if HIDDEN_WIDTH_REFINEMENT:
        return "hidden_width_refined_credible_interval_ea_cv_cyp_gca_submission.csv"
    if BOND_TEMPERATURE_REFINEMENT:
...
def screen(worker: Path) -> dict:
    ...
    if CHANNEL_TIMESCALE_REFINEMENT:
        return screen_channel_timescale_refinement(worker)
    if TRANSITION_ENERGY_REFINEMENT:
    ...
    if HIDDEN_WIDTH_REFINEMENT:
        return screen_hidden_width_refinement(worker)
    if BOND_TEMPERATURE_REFINEMENT:
...
def screen_support_balance_refinement(worker: Path) -> dict:
    """Tune the differentiable-ridge support/query split used in CA training."""
    parent = json.loads(
        (PARENT_INTERVAL_STUDY / "screening_summary.json").read_text(
            encoding="utf-8"
        )
    )
    fractions = (0.60, 0.75, 0.85)
    jobs = [
        (endpoint, candidate, fraction, fold)
        for endpoint in ENDPOINTS for candidate in parent[endpoint]
        for fraction in fractions for fold in SCREEN_FOLDS
    ]
    scores = {
        (endpoint, candidate["rule"], fraction): []
        for endpoint in ENDPOINTS for candidate in parent[endpoint]
        for fraction in fractions
    }
    durations = []
    for number, (endpoint, candidate, fraction, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        config.update({
            "support_fraction": fraction,
            "specialist_objective": "endpoint_only",
        })
        variant = f"{candidate['variant']}_support_{int(fraction * 100):03d}"
        progress(
            "support_balance_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            support_fraction=fraction, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
        ...
        durations.append(time.time() - started)
        scores[(endpoint, candidate["rule"], fraction)].append(float(
            metrics.get("restored_validation_point_ma_st_rae", np.inf)
        ))
    selected = {}
    for endpoint in ENDPOINTS:
        ...
        for candidate in parent[endpoint]:
            choices = []
            for fraction in fractions:
                values = scores[(endpoint, candidate["rule"], fraction)]
                choices.append((float(np.mean(values)), fraction, values))
            score, fraction, values = min(choices)
            config = deepcopy(candidate["config"])
            config.update({
                "support_fraction": fraction,
                "specialist_objective": "endpoint_only",
...
def screen_hidden_width_refinement(worker: Path) -> dict:
    """Test additional CA state capacity without changing graph dynamics."""
    parent = json.loads(
        (PARENT_INTERVAL_STUDY / "screening_summary.json").read_text(
            encoding="utf-8"
        )
    ...
    for number, (endpoint, candidate, scale, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        width = max(4, int(round(float(config.get("hidden_channels", 8)) * scale)))
        config.update({
            "hidden_channels": width,
            "specialist_objective": "endpoint_only",
        })
        variant = f"{candidate['variant']}_width_{width:03d}"
        progress(
            "hidden_width_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            hidden_width_scale=scale, hidden_channels=width, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
...
def screen_generation_depth_refinement(worker: Path) -> dict:
    """Test longer recurrent cellular evolution for each selected rule."""
    parent = json.loads(
        (PARENT_INTERVAL_STUDY / "screening_summary.json").read_text(
            encoding="utf-8"
        )
    ...
    for number, (endpoint, candidate, scale, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        generations = max(8, int(round(float(config.get("generations", 64)) * scale)))
        config.update({
            "generations": generations,
            "specialist_objective": "endpoint_only",
        })
        variant = f"{candidate['variant']}_generations_{generations:04d}"
        progress(
            "generation_depth_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            generation_depth_scale=scale, generations=generations, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
...
def screen_multilag_recurrence_refinement(worker: Path) -> dict:
    """Test short, intermediate, and long recurrent trajectory correlations."""
    parent = json.loads(
        (PARENT_INTERVAL_STUDY / "screening_summary.json").read_text(
            encoding="utf-8"
        )
    ...
    for number, (endpoint, candidate, enabled, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        config.update({
            "multilag_recurrence_signature": enabled,
            "specialist_objective": "endpoint_only",
        })
        suffix = "multilag" if enabled else "core"
        variant = f"{candidate['variant']}_recurrence_{suffix}"
        progress(
            "multilag_recurrence_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            multilag_recurrence_signature=enabled, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
...
def screen_channel_timescale_refinement(worker: Path) -> dict:
    """Test learned channel-specific recurrent update rates."""
    parent = json.loads(
        (PARENT_INTERVAL_STUDY / "screening_summary.json").read_text(
            encoding="utf-8"
        )
    ...
    for number, (endpoint, candidate, enabled, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        config.update({
            "channel_adaptive_timescale": enabled,
            "specialist_objective": "endpoint_only",
        })
        suffix = "adaptive" if enabled else "shared"
        variant = f"{candidate['variant']}_channel_timescale_{suffix}"
        progress(
            "channel_adaptive_timescale_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            channel_adaptive_timescale=enabled, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
...
def screen_initial_scale_refinement(worker: Path) -> dict:
    ...
    for number, (endpoint, candidate, scale, fold) in enumerate(jobs, 1):
        config = deepcopy(candidate["config"])
        init_scale = float(config.get("init_scale", 1.0)) * scale
        config.update({
            "init_scale": init_scale,
            "specialist_objective": "endpoint_only",
        })
        variant = f"{candidate['variant']}_initscale_{int(scale * 100):03d}"
        progress(
            "initial_scale_screen", completed=number - 1,
            total=len(jobs), endpoint=endpoint, rule=candidate["rule"],
            initial_scale_multiplier=scale, init_scale=init_scale, fold=fold,
            estimated_remaining_seconds=(
                np.mean(durations) * (len(jobs) - number + 1)
                if durations else None
            ),
    ...
    for endpoint in ENDPOINTS:
        ...
        for candidate in parent[endpoint]:
            ...
            selected[endpoint].append({
                ...
                "fold_scores": values,
                "interval_loss_beta": candidate.get("interval_loss_beta", 0.0),
                "initial_scale_multiplier": scale,
                "init_scale": init_scale,
                "config": config,
            })
        selected[endpoint].sort(key=lambda item: item["screen_ma_st_rae"])
...
def blind_member(worker: Path, endpoint: str, candidate: dict,
    ...
    env.update({
        ...
        "SME_CHANNEL_ADAPTIVE_TIMESCALE": (
            "1" if candidate["config"].get("channel_adaptive_timescale", False)
            else "0"
        ),
