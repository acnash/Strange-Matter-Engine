#!/usr/bin/env python3
"""Evaluate and package the hybrid-genetic CYP3A4 Graph-CA submission.

The established CIA-EA-CV-CYP-GCA predictions are retained for CYP1A2,
CYP2C9, and CYP2D6.  The genetically selected CYP3A4 specialist is averaged
across five scaffold folds and two seeds.  The sealed holdout is used only for
the final frozen assessment, never for genetic selection.
"""
...
CAMPAIGN = ROOT / "results" / "production_hybrid_genetic_structure_search_cyp3a4_v1"
...
SUBMISSION_NAME = "HGS-CIA-EA-CV-CYP-GCA_OpenADMET_submission.csv"
...
def member_dir(kind: str, fold: int, seed: int) -> Path:
    return CAMPAIGN / kind / "g01_c00" / f"fold_{fold}_seed_{seed}"


def checkpoint(fold: int, seed: int) -> Path:
    return CAMPAIGN / "confirmation" / "g01_c00" / f"fold_{fold}_seed_{seed}" / "model.pt"
...
def worker_environment(kind: str, fold: int, seed: int) -> dict[str, str]:
    ...
    environment.update({
        ...
        "SME_RUN_NAME": relative_output.removeprefix("results/"),
        "SME_DEVICE": "cuda",
...
def run_member(kind: str, fold: int, seed: int) -> Path:
    ...
    filename = ("checkpoint_evaluation_predictions.csv"
                if kind == "sealed_members" else "blinded_test_predictions.csv")
    ...
    with (output / "console.log").open("w", encoding="utf-8") as log:
        subprocess.run(
            [str(PYTHON), str(RUNNER), phase], cwd=ROOT,
...
def run_members(kind: str) -> list[Path]:
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        futures = {pool.submit(run_member, kind, fold, seed): (fold, seed)
...
def sealed_assessment(hybrid: dict[str, float]) -> dict:
    rows = read_csv(BASELINE / "reserved_holdout_predictions.csv")
    output = []
    for row in rows:
        item = dict(row)
        if item["cyp_target"] == "CYP3A4":
            item["predicted_pic50"] = hybrid[item["molecule_id"]]
            item["selected_rules"] = "genetic_fitzhugh_nagumo_g01_c00"
            prediction = float(item["predicted_pic50"])
            low = float(item["credible_interval_low"])
            high = float(item["credible_interval_high"])
            item["soft_threshold_absolute_error"] = max(low - prediction, prediction - high, 0.0)
    ...
    return {
        ...
        "composition": {
            ...
            "CYP3A4": "hybrid-genetic g01_c00, ten-checkpoint mean",
        },
...
def blind_submission(hybrid: dict[str, float], summary: dict) -> Path:
    ...
    for row in baseline_rows:
        row["CYP3A4_pIC50_direct_inhibition"] = hybrid[row["Molecule_Name"]]
    ...
    for row in baseline_rows:
        for endpoint in ENDPOINTS:
            long_rows.append({
                ...
                "source": ("hybrid_genetic_g01_c00_ten_checkpoint_mean"
                           if endpoint == "CYP3A4" else "CIA-EA-CV-CYP-GCA"),
            })
    ...
    manifest = {
        ...
        "finite_predictions": int(sum(
            ...
            for row in baseline_rows for endpoint in ENDPOINTS
        )),
