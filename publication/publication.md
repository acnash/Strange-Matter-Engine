...
All four methods follow the same prediction pathway. The Graph-CA generates a trajectory for each molecule, and summaries of that trajectory form a molecular fingerprint. Ridge regression fits a weighted combination of the fingerprint values to predict pIC50. Its coefficients are calculated analytically from training observations. **All four methods use this fitted ridge readout.** They differ in how they learn the Graph-CA parameters or choose the system that generates the trajectories (Figure 3).

Backpropagation uses prediction error to calculate how each trainable parameter should change. Here, the calculation passes through the ridge fit and backwards through the successive Graph-CA generations. The fixed-rule, hybrid genetic, and genetic-programming methods use backpropagation to train continuous parameters, such as the weights that control how neighbouring atoms influence one another. The evolutionary strategy uses prediction errors from deliberately perturbed parameter values to estimate updates, without backpropagation.

- **Fixed-rule Graph-CA:** a chosen mathematical rule describes how atom states change between generations. Backpropagation learns the numerical weights within that rule and the other Graph-CA components; ridge regression fits the final prediction. The mathematical form of the chosen rule stays fixed during training.
- **Evolutionary strategy:** small random changes are made to the current Graph-CA parameter values, producing a population of candidate models. Each candidate generates trajectories, receives a fitted ridge readout, and is assessed on query molecules. Differences in prediction error indicate which directions of parameter change are useful, and these guide the next update. This method uses ridge regression and does not use backpropagation.
- **Hybrid genetic search:** a genetic algorithm chooses the model's *configuration*, meaning design choices such as the transition-rule family, number of generations, number of dynamical channels, chemical descriptors, and trajectory summaries. Each candidate configuration is trained using backpropagation and a fitted ridge readout. Better-performing configurations are selected and their design choices are recombined or changed to produce new candidates.
- **Genetic programming:** genetic search changes the mathematical expression used to update each atom. For example, it can change how the current atom state, the reaction signal, and differences from neighbouring states are combined using operations such as addition, multiplication, or averaging. Each candidate equation is trained using backpropagation and a fitted ridge readout, then evaluated to guide the next population of equations. The candidate equation remains fixed throughout an individual molecular trajectory.

The following subsections describe these methods and the shared ridge readout in detail. Candidate configurations and equations are selected using scaffold-held-out development data; sealed-holdout and blind labels remain excluded from training and selection.
...
Figure 3A summarises how query-loss gradients train the molecular space-time representation through the differentiable ridge readout and recurrent Graph-CA updates.



#### 2.4.2. Shared ridge readout and its role in training

The analytic ridge readout was shared by all four learning strategies described in Sections 2.4.1 and 2.4.3–2.4.5. In each strategy, molecular trajectory fingerprints from the support subset were used to fit ridge coefficients, which then generated predictions for the query subset. For the fixed-rule backpropagation system (Section 2.4.1), gradients of the query loss passed through this differentiable solve and the recurrent trajectories to update continuous Graph-CA parameters, as illustrated in Figure 3A. The genetic algorithm (Section 2.4.4) and genetic programming (Section 2.4.5) used the same gradient-based fitting within each candidate configuration or symbolic rule, while their outer searches selected candidates using development performance. The evolutionary strategy (Section 2.4.3) instead fitted ridge coefficients for each perturbed parameter candidate and used query error as evolutionary fitness; it estimated parameter updates from those fitness evaluations without differentiating through the ridge solve. Thus, analytic ridge fitting provided the common prediction mechanism, while the strategies differed in how they trained or selected the Graph-CA that generated its input features.
...
The leading GP-CIA-EA-CV-CYP-GCA method evolved executable symbolic programs for the CYP3A4 cellular update while retaining the selected hybrid-search Graph-CA configuration (Figure 3D). The fixed configuration used 128 recurrent generations, 24 hidden channels, comprehensive atom features, multiscale trajectory pooling, dynamic observables, and multiscale transition-energy summaries. Genetic programming therefore isolated changes to the local update equation from changes to graph encoding, state dimension, or training protocol.
...
![Four strategies for learning molecular space-time representations](figures/03_four_molecular_spacetime_learning_strategies.png)

**Figure 3. Four strategies for learning molecular space-time representations.** The shared pipeline converts encoded molecular graphs into retained recurrent trajectories, pools these into molecular fingerprints, and fits an analytic ridge readout on support observations to predict query pIC50 values. **A**, backpropagation through time differentiates query loss through the ridge solve and all recurrent generations; Adam updates continuous Graph-CA parameters within the selected transition-rule family. **B**, the evolutionary strategy evaluates mirrored parameter perturbations, fits a ridge readout for each candidate, and estimates parameter updates from differences in query-error fitness, using Adam-style moments. **C**, hybrid genetic architecture search evolves discrete configuration genomes, including rule family, recurrent depth, channel count, feature profile, and pooling design. Each candidate's continuous parameters are fitted by backpropagation through the differentiable ridge solve before scaffold-held-out development performance determines genetic selection. **D**, genetic programming applies the same inner gradient-based fitting while evolving symbolic transition-rule trees through selection, crossover, and mutation. Development MA-ST-RAE supplies the primary outer fitness for the genetic searches. Solid blue arrows indicate forward computation, dashed purple arrows indicate gradient-based learning, and green arrows indicate evolutionary updates or genetic search. The candidate transition rule remains shared across atoms and generations within each trajectory; genetic changes occur between candidates. Sealed-holdout and blind labels are excluded from learning and model selection.
