...
Here (eta_0) is the intercept and (eta_j) is the coefficient for fingerprint component (j).
...
After centring (y) and the columns of (Z), ordinary least squares chooses (eta) to minimise the residual sum of squares:
...
then predictions return to the pIC50 scale by adding (ar y_{\rm train}).
...
4. fit ridge coefficients (eta); and
...
1. hold (	heta) fixed and fit (eta);
2. hold or temporarily fix (eta) and update (	heta) using backpropagation;
...
Backpropagation then computes gradients for both (	heta) and (eta). This is mathematically convenient when the fingerprint components are differentiable. It changes the optimisation procedure, while the readout remains linear and L2-regularised.
